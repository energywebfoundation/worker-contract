...
describe("VotingFacet", function () {
  ...
  const timeframes = [
    ...
    {
      input: ethers.utils.formatBytes32String("MATCH_INPUT_1"),
      output: ethers.utils.formatBytes32String("REPLAYED_MATCH_OUTPUT_1"),
    },
    {
  ...
  it("should not reveal winners before the end of vote", async () => {
    ...
    expect(await matchVoting.getWinners(timeframes[0].input)).to.be.empty;
    ...
    expect(await matchVoting.getWinners(timeframes[0].input)).to.be.deep.equal([
      worker1.address,
      worker2.address,
    ]);
  });
  ...
  it("should not reveal winningMatches before the end of vote", async () => {
    ...
    expect(await matchVoting.getWinningMatch(timeframes[0].input)).to.equal(
  ...
  it("reward should be paid after replenishment of funds", async () => {
    ...
    await expect(
      matchVoting.connect(faucet).replenishRewardPool({
        value: DEFAULT_REWARD_AMOUNT.mul(3),
      })
    )
      .to.emit(matchVoting, "Replenished")
  ...
  it("reverts when non owner tries to cancel expired votings", async () => {
    ...
    await expect(
      matchVoting.connect(worker1).cancelExpiredVotings()
