import {createMerkleTree, hash} from '../src';
...
describe('#createMerkleTree', () => {
  ...
  const hashGeneration = (generation: Match) => hash(JSON.stringify(generation), 'EXCESS_GENERATION');
  const hashConsumption = (consumption: Match) => hash(JSON.stringify(consumption), 'LEFTOVER_CONSUMPTION');

  const createLeavesFromMatchingResult = ({
    matches = [],
    generations = [],
    consumptions = [],
  }: { matches?: Match[], consumptions?: Match[], generations?: Match[] }) =>
    [
  ...
  describe('Match verifications', () => {
    ...
    it('should be able to verify if match is not in the tree', () => {
      const [matchLeaf] = createLeavesFromMatchingResult({matches: [{id: 1}]});
      const [otherMatchLeaf] = createLeavesFromMatchingResult({matches: [{id: 2}]});
  ...
  describe('Leftover consumption verifications', () => {
    it('should be able to verify if leftover consumption is in the tree', () => {
      const leaves = createLeavesFromMatchingResult({consumptions: [{volume: '1'}, {volume: '2'}]});
    ...
    it('should be able to verify if leftover consumption is not in the tree', () => {
      const leaves = createLeavesFromMatchingResult({consumptions: [{volume: '1'}, {volume: '2'}]});
      const [otherLeaf] = createLeavesFromMatchingResult({consumptions: [{volume: '3'}]});
  ...
  describe('Excess generation verifications', () => {
    it('should be able to verify if excess generation is in the tree', () => {
      const leaves = createLeavesFromMatchingResult({generations: [{volume: '1'}, {volume: '2'}]});
    ...
    it('should be able to verify if excess generation is not in the tree', () => {
      const leaves = createLeavesFromMatchingResult({generations: [{volume: '1'}, {volume: '2'}]});
      const [otherLeaf] = createLeavesFromMatchingResult({generations: [{volume: '3'}]});
  ...
  it('should work for larger trees', () => {
    const leaves = createLeavesFromMatchingResult({
      ...
      consumptions: new Array(100).fill(null).map((_, index) => ({consumption: index})),
      generations: new Array(100).fill(null).map((_, index) => ({generation: index})),
    });
